...
| Ça marche en navigation privée, pas dans la fenêtre ordinaire | OnlyOffice installe un **service worker** sur son domaine. Il survit au rechargement et continue de servir l'échec qu'il a mis en cache, même une fois le bloqueur retiré — un Ctrl+Maj+R ne le désinstalle pas. F12 → Application → Service Workers → *Unregister*, puis *Clear site data*, **sur le domaine de l'éditeur** et non sur celui de la plateforme. |
